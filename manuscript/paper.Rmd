...
systolic blood pressure. Weekday sleep duration between 6 and 8 hours
...
Table 1 displays the estimated coefficients derived from a multiple
regression analysis involving variables. We only show sleep during
weekdays, age, total cholesterol, and the interaction effect between
total cholesterol and age. Notably, the findings reveal that an increase
of one year in age is associated with a marginal decrease in systolic
blood pressure (SBP) by an amount of 0.04mmHg. However, this observed
reduction is deemed statistically insignificant within the confines of
this model. Conversely, an increase of one unit in total cholesterol is
linked to a more pronounced reduction in SBP by a factor of 3.5mmHg,
which attains statistical significance. Furthermore, the interaction
between age and total cholesterol yields a statistically significant
coefficient estimate of 0.11. This interaction effect suggests that as
both age and total cholesterol concurrently increase by one unit, there
is an associated elevation in SBP by a factor of 0.11mmHg. Additionally,
the coefficients attributed to sleep duration bear analogous
interpretations to those observed in the univariate scenario.
Specifically, weekday sleep durations ranging between 6 and 8 hours
demonstrate an average decrease of 3.0mmHg in systolic blood pressure in
comparison to the reference duration of 6 hours of sleep. Conversely,
durations exceeding 8 hours exhibit a decrease of 1.4mmHg in systolic
blood pressure as compared to the 6-hour reference duration.

```{=tex}

\begin{table}[H]
\caption{\label{tab:multreg} Sleep, Age and Cholesterol Coefficients for Model 5}
\centering
