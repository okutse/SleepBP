...
Univariate regression analyses were performed on all potentially confounding covariates excluding hypertension. Model 1 was fitted to the data including the exposure variables as well as all potentially confounding covariates from univariate regression analysis. Model 2 was fitted to the same set of covariates including an interaction term between age and total cholesterol levels. Model 3 was fitted to the data including only covariates that had a statistically significant association with SBP in univariate analyses excluding HDL, hemoglobin, diabetes, survey cycle, and alcohol consumption. Model 4 included similar covariates as Model 3 but examined, in addition, all possible two-way interactions between the covariates. Model 5 included the same set of covariates as Model 3 in addition to an interaction term between age and total cholesterol levels.
...
All models fitted to the data in this investigation were nested and evaluations were based on information criteria including Akaike Information Criteria (AIC) and Bayesian Information Criteria (BIC). [BIC] Additionally, we also used performance measures including the mean squared error (MSE), r-squared, and adjusted r-squared. The likelihood ratio test (LRT) was used in competing model comparisons testing the null hypothesis that the nested models are equivalent, and the extra parameters in the larger model do not improve the fit significantly, that is, these terms have effect sizes equivalent to 0. \@ref(tab:tabmodel) summarizes the model performance metrics compared for all models implemented in this investigation.
...
Table \@ref(tab:tabmodel) summarizes the performance metrics across the
different models fitted to the data in our investigation.
...
\toprule
  & df & AIC & BIC & MSE & $R^2$ & Adjusted $R^2$\\
\midrule
...
\addlinespace
Model 6 & 41 & 15888.38 & 16117.55 & 173.69 & 0.36 & 0.34\\
Model 7 & 25 & 15877.41 & 16017.15 & 2657.01 & 0.35 & 0.34\\
\bottomrule
